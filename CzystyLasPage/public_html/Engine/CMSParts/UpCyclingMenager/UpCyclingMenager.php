...
/**
 * Description of UpCyclingMenager
 *
 * @author Lukasz
 */
class UpCyclingMenager extends DatabaseEditor {
    
    public function __construct($_colNames = array(), $_target) {
        parent::__construct($_colNames, $_target, 'inforest');

        $this->DeleteButton = '<button type="submit" name="function" value="delete_upcycling" class="newsDeleteItem">-</button>';
        $this->AddButton = '<button type="submit" name="function" value="add_upcycling" class="newsOK">OK</button>';
        $this->EditButton = '<button type="submit" name="function" value="edit_upcycling" class="newsOK">OK</button>';
    }
    ...
    public function Add($_params = array()) {
        ...
        $id = mysql_insert_id();
        $q = "INSERT INTO `czysty-las-database`.`upcycling` (`Id`, `Photo`, `Title`, `Description`) VALUES (NULL, '$id', '$_params[1]', '$_params[2]')";
        $ins = mysql_query($q);
    }
    ...
    public function Delete($_param) {
       ...
       $q = "DELETE FROM `czysty-las-database`.`upcycling` WHERE `upcycling`.`Id` = ".$_param;
       $ins = mysql_query($q);        
    }

    public function Edit($_params = array()) {
        
        $q = "SELECT `Photo` FROM `upcycling` WHERE `Id` = " . $_params[0];
        $ins1 = mysql_query($q);
        ...
        if ($_params[1]['tmp_name'] != NULL) {
            ...
            $q = "UPDATE `czysty-las-database`.`upcycling` SET `Photo` = '" . mysql_insert_id() . "', `Title` = '$_params[2]', `Description` = '$_params[3]' WHERE `upcycling`.`Id` = " . $_params[0];
        } else {
            $q = "UPDATE `czysty-las-database`.`upcycling` SET `Title` = '$_params[2]', `Description` = '$_params[3]' WHERE `upcycling`.`Id` = " . $_params[0];
        }
    ...
    public function Show() {
        ...
        if (!isset($_GET['add']) && !isset($_GET['Id'])) {
            $q = "SELECT * FROM `upcycling`";
            $ins = mysql_query($q);
            echo '<div class="inforestContent">';
            echo '<a class="inforestAddItem" href="CMS.php?function=upcycling&add=true">Dodaj</a>';
            ...
            while ($print = mysql_fetch_array($ins)) {
                ...
                echo '<a class="inforestItem" href="CMS.php?function=upcycling&Id=' . $print['Id'] . '">';
            ...
            if ($id == 0) {
                ...
                $q = "SELECT * FROM `upcycling` WHERE `Id` = " . $id;
                $Edit = mysql_query($q);
